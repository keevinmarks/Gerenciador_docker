...
import type { Computer } from "../../types/usersTypes.js";
...
const computerSchema = z.object({
    ...
    mac_computer: z.string().min(2, {message: "O mac do computador deve ter pelo menos 12 caracteres"}),
    ...
    exit_date: z.string().min(2, {message: "A data de saída deve conter pelo menos 10 caracteres"}).optional().nullable(),
    reason: z.string().min(2, {message: "O motivo deve ter pelo menos 2 caracteres"}),
    return_date: z.string().min(2, {message: "A data de retorno deve conter pelo menos 10 caracteres"}).optional().nullable()
});
...
computersRouter.post("/", async (req, res) => {
    ...
    if(!validate_computer.success){
        ...
        return res.status(400).json({message: "Dados inválidos na inserção", success: false});
    }
    ...
    try{
        ...
        console.log("Erro na inserção de computador " + error);
...
computersRouter.put("/", async (req, res) => {
    ...
    if(!validate_computers.success){
        console.log("Dados inválidos");
        return res.status(400).json({message: "Dados inválidos na atualização"})
    }
    ...
    try{
        ...
        console.log("Erro na atualização do computador " + error);
...
computersRouter.get("/", async (req, res) => {
    ...
    try{
        ...
        const formattedRows = rows.map((row:Computer) => {
            const exitDate = row.exit_date;
            const returnDate = row.return_date;

            const formatDate = (dataValue: any) => {
                if(dataValue instanceof Date){
                    return dataValue.toISOString().substring(0, 10);
                }
                return dataValue
            }
            ...
            return {
                ...row,
                exit_date: formatDate(row.exit_date),
